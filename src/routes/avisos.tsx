...
import { dataAvisoBR, useAvisos } from "@/lib/avisos-db";
...
function Avisos() {
  const { avisos, carregando } = useAvisos();
        ...
        {avisos.map((a) => (
          <article key={a.id} className="surface-card p-4">
            <div className="flex items-start justify-between gap-2">
              ...
              {a.fixado_home ? <Pin className="mt-1 size-4 shrink-0 text-primary" /> : null}
            </div>
            <p className="mt-1 text-sm">{a.descricao}</p>
            <p className="mt-3 text-xs text-soft">
              Publicado por {a.autor} · {dataAvisoBR(a)}
            </p>
          </article>
        ))}
